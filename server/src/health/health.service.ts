...
export class HealthService {
  ...
  private async getUserMetrics() {
    try {
      ...
      roleQuery.forEach((row) => {
        byRole[row.user_role as UserRole] = parseInt(row.count);
      });
  ...
  private async getTaskMetrics() {
    try {
      ...
      statusQuery.forEach((row) => {
        byStatus[row.task_status as TaskStatus] = parseInt(row.count);
      });
      ...
      priorityQuery.forEach((row) => {
        byPriority[row.task_priority as TaskPriority] = parseInt(row.count);
      });

      // Taux de completion
      const completedTasks = byStatus[TaskStatus.DONE] || 0;
  ...
  private getSystemMetrics() {
    try {
      ...
      // CPU usage approximation (basé sur le temps de process)
  ...
  private async getServiceMetrics() {
    try {
      ...
      // Test du service auth (vérifier si on peut créer un token JWT)
      let authStatus: 'healthy' | 'unhealthy' = 'healthy';
      try {
        // Simple test - vérifier que les variables d'environnement JWT sont présentes
  ...
  // Méthode simple pour le health check basique
